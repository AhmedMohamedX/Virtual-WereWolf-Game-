...
public class CurrentRoomCanvas : MonoBehaviour {
    ...
    private float prematchCountdown = 10.0f;
    ...
    private void RPC_CountDown(PhotonPlayer photonPlayer)
    {
        ...
        waitingforatherplayers.text = "your game will begin in " + myBlubb;
    ...
    {             
        ...
        PhotonView.RPC("RPC_DisableNameothers", PhotonTargets.AllBuffered, PhotonNetwork.player);
    }


    [PunRPC]
    private void RPC_DisableNameothers(PhotonPlayer photonPlayer)
    {
        edittext = GameObject.FindGameObjectsWithTag("editname");
        foreach (var item in edittext)
        {
            if (!item.name.Equals("PlayerName" + photonPlayer.ID))
            {
                textname = item.GetComponent<InputField>();
                textname.enabled = false;
            }
